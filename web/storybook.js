...
var tap_default = tap;
// client/fixtures/basic_components.f.ts
var exports_basic_components_f = {};
__export(exports_basic_components_f, {
  SPTInputStories: () => {
    {
      return SPTInputStories;
    }
  },
  PersonBubbleStories: () => {
    {
      return PersonBubbleStories;
    }
  },
  ...
  FixedInputStories: () => {
    {
      return FixedInputStories;
    }
  },
  AssessmentOutputsViewStories: () => {
    {
      return AssessmentOutputsViewStories;
    }
  },
  AssessmentInputsViewStories: () => {
    {
      return AssessmentInputsViewStories;
    }
  }
});
...
function coerceNumbers(values3) {
  return values3 instanceof TypedArray ? values3 : map6(values3, coerceNumber, Float64Array);
}
...
function map6(values3, f, type5 = Array) {
  return values3 == null ? values3 : values3 instanceof type5 ? values3.map(f) : type5.from(values3, f);
}
function slice2(values3, type5 = Array) {
  return values3 instanceof type5 ? values3.slice() : type5.from(values3);
}
...
function take(values3, index2) {
  return map6(index2, (i) => values3[i]);
}
...
var conicProjection2 = function(createProjection2, kx2, ky2) {
  ...
  return {
    type: (options9) => {
      ...
      const projection3 = type5(options9);
      if (parallels != null) {
...
function createScaleOrdinal(key, channels, { type: type5, interval: interval10, domain, range: range5, scheme: scheme28, unknown, ...options12 }) {
  interval10 = maybeRangeInterval(interval10, type5);
  if (domain === undefined)
    domain = inferDomain2(channels, interval10, key);
  let hint;
  ...
  return createScaleO(key, ordinal().unknown(unknown), channels, { ...options12, type: type5, domain, range: range5, hint });
}
...
var createScale = function(key, channels = [], options13 = {}) {
  const type5 = inferScaleType(key, channels, options13);
  if (options13.type === undefined && options13.domain === undefined && options13.range === undefined && options13.interval == null && key !== "fx" && key !== "fy" && isOrdinalScale({ type: type5 })) {
...
var coerceSymbols = function(values4) {
  return map6(values4, maybeSymbol);
};
...
var legendOpacity = function({ type: type5, interpolate: interpolate3, ...scale }, { legend = true, color: color10 = rgb(0, 0, 0), ...options26 }) {
  if (!interpolate3)
  ...
  return legendColor({ type: type5, ...scale, interpolate: interpolateOpacity(color10) }, { legend, ...options26 });
};
...
var PersonBubble = {
  view: ({ attrs: { name, color: color10 } }) => {
    return import_mithril2.default("span.person-bubble", { title: name, style: { "background-color": color10, color: "#eee" } }, import_mithril2.default("span", name.split(" ").slice(0, 2).map((x2) => x2[0])));
  }
};
var LabeledNumber = {
...
class Value {
  constructor(value_, context15, customGetValue = undefined) {
    this.value_ = value_;
...
class ChainArbitrary extends Arbitrary {
  ...
  shrink(value5, context15) {
    if (this.isSafeContext(context15)) {
      return (!context15.stoppedForOriginal ? this.arb.shrink(context15.originalValue, context15.originalContext).map((v) => this.valueChainer(v, context15.clonedMrng.clone(), context15.clonedMrng, context15.originalBias)) : Stream.nil()).join(context15.chainedArbitrary.shrink(value5, context15.chainedContext).map((dst) => {
...
class FilterArbitrary extends Arbitrary {
  ...
  shrink(value5, context15) {
    return this.arb.shrink(value5, context15).filter(this.bindRefinementOnValue);
  }
  refinementOnValue(v) {
...
class NoBiasArbitrary extends Arbitrary {
  ...
  shrink(value5, context15) {
    return this.arb.shrink(value5, context15);
  }
  noBias() {
    return this;
  }
...
class Property {
  ...
  generate(mrng, runId) {
    const value5 = this.arb.generate(mrng, runId != null ? runIdToFrequency(runId) : undefined);
    return noUndefinedAsContext(value5);
  }
  shrink(value5) {
    if (value5.context === undefined && !this.arb.canShrinkWithoutContext(value5.value_)) {
      return Stream.nil();
    }
    const safeContext = value5.context !== UndefinedContextPlaceholder ? value5.context : undefined;
...
var MersenneTwister = function() {
  function MersenneTwister2(states2, index2) {
    this.states = states2;
    this.index = index2;
  }
  MersenneTwister2.twist = function(prev) {
    ...
    var y2 = (mt[MersenneTwister2.N - 1] & MersenneTwister2.MASK_UPPER) + (mt[0] & MersenneTwister2.MASK_LOWER);
    mt[MersenneTwister2.N - 1] = mt[MersenneTwister2.M - 1] ^ y2 >>> 1 ^ -(y2 & 1) & MersenneTwister2.A;
    return mt;
  };
...
function addArrayIntToNew(arrayIntA, arrayIntB) {
  ...
  for (var indexA = dataA.length - 1, indexB = dataB.length - 1;indexA >= 0 || indexB >= 0; --indexA, --indexB) {
    ...
    data2.push(current >>> 0);
    reminder = ~~(current / 4294967296);
  }
  if (reminder !== 0) {
    data2.push(reminder);
  }
  return { sign: arrayIntA.sign, data: data2.reverse() };
}
function addOneToPositiveArrayInt(arrayInt) {
  arrayInt.sign = 1;
  var data2 = arrayInt.data;
  for (var index2 = data2.length - 1;index2 >= 0; --index2) {
    if (data2[index2] === 4294967295) {
      data2[index2] = 0;
    } else {
      data2[index2] += 1;
      return arrayInt;
...
var isStrictlySmaller = function(dataA, dataB) {
  ...
  for (var index2 = 0;index2 < maxLength; ++index2) {
    var indexA = index2 + dataA.length - maxLength;
    var indexB = index2 + dataB.length - maxLength;
    var vA = indexA >= 0 ? dataA[indexA] : 0;
    var vB = indexB >= 0 ? dataB[indexB] : 0;
    if (vA < vB)
      return true;
...
function trimArrayIntInplace(arrayInt) {
  var data2 = arrayInt.data;
  var firstNonZero = 0;
  for (;firstNonZero !== data2.length && data2[firstNonZero] === 0; ++firstNonZero) {
  }
  if (firstNonZero === data2.length) {
    arrayInt.sign = 1;
    arrayInt.data = [0];
    return arrayInt;
  }
  data2.splice(0, firstNonZero);
  return arrayInt;
}
...
class QualifiedParameters {
  ...
  toParameters() {
    const orUndefined = (value5) => value5 !== null ? value5 : undefined;
    const parameters = {
      seed: this.seed,
...
class UnbiasedProperty {
  ...
  shrink(value5) {
    return this.property.shrink(value5);
  }
  run(v, dontRunHook) {
...
class Random {
  ...
  nextDouble() {
    ...
    return (a2 * Random.DBL_FACTOR + b) * Random.DBL_DIVISOR;
  }
}
...
var tossNext = function(generator, rng, index2) {
  rng.unsafeJump();
...
function pathWalk(path3, initialValues, shrink) {
  ...
  for (const s2 of segments.slice(1)) {
    ...
    if (valueToShrink == null) {
      throw new Error(`Unable to replay, got wrong path=${path3}`);
    }
    values4 = shrink(valueToShrink).drop(s2);
  }
  return values4;
}
...
var toProperty = function(generator, qParams) {
  const prop4 = !Object.prototype.hasOwnProperty.call(generator, "isAsync") ? new Property(generator, () => true) : generator;
  return qParams.unbiased === true ? new UnbiasedProperty(prop4) : prop4;
};
...
function shrinkInteger(current, target, tryTargetAsap) {
  ...
  return realGap > 0 ? stream3(shrinkDecr()) : stream3(shrinkIncr());
}
...
class IntegerArbitrary extends Arbitrary {
  ...
  generate(mrng, biasFactor) {
    ...
    return new Value(mrng.nextInt(range5.min, range5.max), undefined);
  }
  canShrinkWithoutContext(value5) {
    return typeof value5 === "number" && safeNumberIsInteger(value5) && !safeObjectIs(value5, -0) && this.min <= value5 && value5 <= this.max;
  }
  shrink(current, context15) {
    if (!IntegerArbitrary.isValidContext(current, context15)) {
      const target = this.defaultTarget();
      return shrinkInteger(current, target, true);
    }
    if (this.isLastChanceTry(current, context15)) {
      return Stream.of(new Value(context15, undefined));
    }
    return shrinkInteger(current, context15, false);
  }
  defaultTarget() {
  ...
  isLastChanceTry(current, context15) {
    if (current > 0)
      return current === context15 + 1 && current > this.min;
    if (current < 0)
      return current === context15 - 1 && current < this.max;
    return false;
  }
  static isValidContext(current, context15) {
    if (context15 === undefined) {
      return false;
    }
    if (typeof context15 !== "number") {
      throw new Error(`Invalid context type passed to IntegerArbitrary (#1)`);
    }
    if (context15 !== 0 && safeMathSign(current) !== safeMathSign(context15)) {
...
function decomposeFloat(f) {
  ...
  const exponent5 = exponentBits === 0 ? -126 : exponentBits - 127;
  let significand = exponentBits === 0 ? 0 : 1;
  significand += significandBits / 2 ** 23;
  significand *= signBit === 0 ? 1 : -1;
  return { exponent: exponent5, significand };
}
var indexInFloatFromDecomp = function(exponent5, significand) {
  if (exponent5 === -126) {
    return significand * 8388608;
  }
  return (exponent5 + 127) * 8388608 + (significand - 1) * 8388608;
};
function floatToIndex(f) {
  ...
  const exponent5 = decomp.exponent;
  const significand = decomp.significand;
  if (f > 0 || f === 0 && 1 / f === safePositiveInfinity) {
    return indexInFloatFromDecomp(exponent5, significand);
  } else {
    return -indexInFloatFromDecomp(exponent5, -significand) - 1;
  }
}
function indexToFloat(index2) {
  ...
  const exponent5 = -125 + (postIndex >> 23);
  const significand = 1 + (postIndex & 8388607) / 8388608;
  return significand * 2 ** exponent5;
}
...
var unmapperFloatToIndex = function(value5) {
...
function float(constraints = {}) {
  ...
  if (noNaN) {
    return integer({ min: minIndex2, max: maxIndex2 }).map(indexToFloat, unmapperFloatToIndex);
  }
  const minIndexWithNaN = maxIndex2 > 0 ? minIndex2 : minIndex2 - 1;
  const maxIndexWithNaN = maxIndex2 > 0 ? maxIndex2 + 1 : maxIndex2;
  return integer({ min: minIndexWithNaN, max: maxIndexWithNaN }).map((index2) => {
    if (index2 > maxIndex2 || index2 < minIndex2)
      return safeNaN;
    else
    ...
    if (typeof value5 !== "number")
      throw new Error("Unsupported type");
    if (safeNumberIsNaN(value5))
      return maxIndex2 !== maxIndexWithNaN ? maxIndexWithNaN : minIndexWithNaN;
    return floatToIndex(value5);
  });
}
  ...
  stories: [
    {
      ...
      attrs: {
        label: "Sample Label",
        number: sample(float({ min: -1e9, max: 1e9 }))[0]
      }
    },
    {
      name: "Dollar Value",
    ...
    },
    {
      name: "Percentage",
      attrs: {
        number: sample(float({ min: 0, max: 100 }))[0],
        label: "Mean Quantile",
        postunit: "%"
      }
    }
  ...
  stories: [
    {
      ...
      attrs: {
        ...
        update: (x2) => console.log(x2)
      }
    }
  ]
};
var FixedInputStories = {
  name: "FixedInputView",
  ...
  stories: [
    {
      ...
      attrs: {
        name: "variable",
        input: sample(integer({ min: -1e4, max: 1e5 }))[0],
        update: (x2) => console.log(x2)
      }
    }
  ]
};
var PersonBubbleStories = {
  name: "PersonBubble",
...
var AssessmentInputsViewStories = {
...
var AssessmentOutputsViewStories = {
...
var ShowFixtures = {
  view({ attrs }) {
    return import_mithril3.default("div.fixture", import_mithril3.default("h1.title", attrs.name), import_mithril3.default("div.stories", attrs.stories.map((story) => import_mithril3.default("div.story", import_mithril3.default("h3.story-name", story.name), import_mithril3.default("div.rendering", { style: story.style ?? undefined }, import_mithril3.default(attrs.component, story.attrs))))));
  }
};
...
import_mithril4.default.mount(document.getElementById("app"), {
  view() {
    console.log("fixtures", values_default(exports_basic_components_f));
    return import_mithril4.default("div.storybook", import_mithril4.default("div.sidebar", import_mithril4.default("div.nav-bar", values_default(exports_basic_components_f).map((x2) => import_mithril4.default("div.nav-item", x2.name)))), import_mithril4.default("div.main-panel", values_default(exports_basic_components_f).map((fixture) => import_mithril4.default(ShowFixtures, fixture))));
  }
});
